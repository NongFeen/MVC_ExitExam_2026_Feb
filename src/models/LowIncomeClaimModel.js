...
const file = path.join(__dirname, "LowIncomeClaimsData.json");
...
const calculateCompensation = (claimantId, claimId, moneyModel) => {
  try {
    ...
    if (claimant.claimantType !== "low-income") {
      throw new Error("not Low-Income Claim Model");
    }

    const monthlyIncome = claimant.monthlyIncome;
    if (monthlyIncome >= 6500) {
      throw new Error("not low-income ");
    }

    const compensationAmount = 6500;
